...
import {CatResponse} from '../src/interfaces/Cat';
import DBMessageResponse from '../src/interfaces/DBMessageResponse';
import {Types} from 'mongoose';

const getCat = (url: string | Function): Promise<CatResponse[]> => {
    ...
    request(url)
      ...
      .expect(200, (err, response) => {
        if (err) {
          ...
          const cats: CatResponse[] = response.body;
          cats.forEach((cat) => {
            expect(cat).toHaveProperty('id');
            expect(cat).toHaveProperty('name');
            expect(cat.owner.id).not.toBeUndefined();
...
const postCat = (
  ...
  return new Promise((resolve, reject) => {
    request(url)
      ...
      .attach('cat', 'test/' + pic)
      .field('name', 'Test Cat ' + new Date().toLocaleDateString('fi-FI'))
    ...
    request(url)
      ...
      .expect(200, (err, response) => {
        if (err) {
          ...
          const cat: DBMessageResponse = response.body;
          expect((cat.data as CatResponse).name).toBe(newName);
          expect(cat.message).not.toBe('');
          resolve(cat);
        }
      });
    ...
    request(url)
      ...
      .expect(200, (err, response) => {
        if (err) {
          ...
          const cat: CatResponse[] = response.body;
