...
import {User, UserResponse, UserTest} from '../src/interfaces/User';
...
const getUser = (url: string | Function): Promise<UserResponse[]> => {
    ...
    request(url)
      ...
      .expect(200, (err, response) => {
        if (err) {
          ...
          expect(response.body.data).not.toHaveProperty('password');
          expect(response.body.data).not.toHaveProperty('role');
          expect(response.body.data.username).toBe(newValue);
          resolve(response.body);
        }
      });
  });
    ...
    request(url)
      ...
      .expect(200, (err, response) => {
        if (err) {
          ...
          const user: UserResponse = response.body.user;
          expect(user).toHaveProperty('username');
          expect(user).toHaveProperty('email');
          expect(user).not.toHaveProperty('role');
          expect(user).toHaveProperty('id');
